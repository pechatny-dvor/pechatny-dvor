...
//                        p.$jot('.jt-content, .jt-title').css('width', 'auto');
   ...
   */
    ...
    initDateV2Inputs: function() {
        ...
        liteModeInputs.forEach(function(input) {
            try {
                var imask = new Inputmask({
                    alias: 'datetime',
                    inputFormat: input.placeholder,
                    jitMasking: true,
                });
                imask.mask(input);
            } catch (e) {
    ...
    initRadioInputV2: function (labelElArr) {
        ...
        var clearCheck = function(input) {
            var allRadioElements = document.querySelectorAll('li.form-line[data-type="control_radio"] input[type="radio"]');
            for(var i = 0 ; i < allRadioElements.length; i++){
                if(allRadioElements[i] === input){
                  continue;
                }
                if(allRadioElements[i].getAttribute('checked-before')){
                  allRadioElements[i].setAttribute('checked-before', 0);
                }
            }
        };
        ...
        for (var i = 0; i < labelElArr.length; i++) {
            labelElArr[i].addEventListener('click', function (e) {
                if(e.target.previousElementSibling){
                  clearCheck(e.target.previousElementSibling);
                }
                checkBeforeFunc(e.target);
            });
            // labelElArr[i].addEventListener('keydown', function (e) {
            //     checkBeforeFunc(e.target);
            // });
            // labelElArr[i].addEventListener('keyup', function (e) {
            //     e.preventDefault();
            //     var charCode = e.which || e.keyCode || e.charCode;
            ...
            form.observe('submit', function (e) {
                ...
                if (JotForm.compact && JotForm.imageSaved == false) {
                    ...
                    $(document).observe('image:loaded', function () {
                        ...
                        block.insert('<table height="100%" width="100%"><tr><td align="center" valign="middle" style="font-family:Verdana;color:#fff;font-size:16px;">Please 222222 Wait...</td></tr></table>');
                        setTimeout(function () {
                            form.submit();
                        }, 1000);
                    });
